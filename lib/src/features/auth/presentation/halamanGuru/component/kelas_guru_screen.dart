...
class _KelasGuruScreenState extends ConsumerState<KelasGuruScreen> {
  ...
  List<Map<String, dynamic>> get filteredKelasList {
    ...
    return _kelasList.where((k) {
      ...
      return '${k['nama']} ${k['mataPelajaran']} ${k['ruangan']} ${k['waliKelas']}'
          .toLowerCase()
          .contains(q);
    }).toList();
  }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              _searchQuery.isEmpty ? 'Belum ada kelas' : 'Tidak ditemukan',
              ...
              Row(
                children: [
                  ...
                  PopupMenuButton(
                    ...
                    onSelected: (v) {
                      if (v == 'view')
                        _showDetail(context, k);
                      else if (v == 'edit')
                        _showEditDialog(context, k);
                      else if (v == 'delete')
  ...
  Widget _buildInfo(IconData icon, String text, bool isDark) => Row(
    children: [
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Divider(),
              const SizedBox(height: 16),
              _buildDetailRow(Icons.class_, 'Nama Kelas', k['nama'], isDark),
              ...
              const SizedBox(height: 12),
              _buildDetailRow(Icons.schedule, 'Jadwal', k['jadwal'], isDark),
              const SizedBox(height: 12),
              _buildDetailRow(Icons.room, 'Ruangan', k['ruangan'], isDark),
              const SizedBox(height: 12),
              _buildDetailRow(
                Icons.person,
                'Wali Kelas',
  ...
  void _showAddDialog(BuildContext context) =>
      _showFormDialog(context, 'Tambah Kelas Baru', null);
  void _showEditDialog(BuildContext context, Map<String, dynamic> k) =>
      _showFormDialog(context, 'Edit Kelas', k);
                ...
                setState(() {
                  if (k == null) {
                    _kelasList.add({
                      ...
                      'nama': nama.text,
                      'mataPelajaran': mapel.text,
                      'jumlahSiswa': int.parse(siswa.text),
                      'jadwal': jadwal.text,
                    ...
                    k['nama'] = nama.text;
                    k['mataPelajaran'] = mapel.text;
                    k['jumlahSiswa'] = int.parse(siswa.text);
                    k['jadwal'] = jadwal.text;
                    k['ruangan'] = ruangan.text;
                    k['waliKelas'] = wali.text;
                  }
                });
                Navigator.pop(context);
                  ...
                  SnackBar(
                    content: Row(
                      children: [
                        ...
                        const SizedBox(width: 12),
                        Text(
                          k == null ? 'Kelas ditambahkan' : 'Kelas diperbarui',
                        ),
                      ],
                    ),
  ...
  void _showDeleteDialog(BuildContext context, Map<String, dynamic> k) {
    showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        content: Text('Yakin hapus kelas "${k['nama']}"?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Batal'),
